            ...
            <tbody>
              <template v-for="item in filteredItems" :key="item.brd_id">
                <tr>
                  ...
                  <td style="border-bottom: none !important">
                    <div class="d-flex">
                      <v-btn color="green" variant="text" @click="editRestaurantDish(item)" icon>
...
export default {
  ...
  methods: {
    ...
    deleteMainImageFile() {
      this.isSending = true;
      axios
        .delete(`/biryani-restaurant-dishes/${this.propertyDataToMainImage.brd_id}/main-image`)
        .then((response) => {
    ...
    getRestaurantMasterData() {
      axios.get(`/biryani-restaurant-masters`).then((response) => {
    ...
    getRestaurantDishData() {
      axios.get(`/biryani-restaurant-dishes`).then((response) => {
        const data = response.data.data;
        this.items = data;
      });
    },
