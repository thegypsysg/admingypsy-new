    ...
    <v-sheet class="py-6 px-4 mt-10" border rounded width="100%">
      ...
      <v-row>
        <v-col cols="12">
            ...
            <tbody>
              <template v-for="(item, index) in platformFee" :key="index">
                ...
                <tr class="platform-fee-table-body">
                  ...
                  <td>
                    <div class="d-flex">
                      <v-btn color="green" variant="text" @click="editPlatformFee(item)" icon>
          ...
          <empty-state
            v-if="!isLoading && (!platformFee || platformFee.length === 0)"
            title="No Data Found"
...
const saveEdit = () => {
  if (valid.value) {
    ...
    axios
      .put(`/platform-fee/update/${platformFeeForm.value.pf_id}`, payload)
      .then((response) => {
        const data = response.data;
...
const saveData = async () => {
  if (formRef.value) {
    ...
    if (isValid) {
      isSending.value = true;
      axios
        .post(`/platform-fee/save`, platformFeeForm.value)
        .then((response) => {
          const data = response?.data;
          successMessage.value = data?.message;
          isSuccess.value = true;
...
const deletePlatformFee = () => {
  ...
  axios
    .delete(`/platform-fee/${idPlatformFee.value}`)
    .then((response) => {
      const data = response.data;
      successMessage.value = data.message;
      isSuccess.value = true;
