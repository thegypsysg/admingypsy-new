    ...
    <v-sheet class="py-6 px-4 mt-6" border rounded width="100%">
      ...
      <v-row>
        <v-col cols="12">
          <v-table class="country-table">
            ...
            <tbody>
              <tr class="country-table-body" v-for="item in filteredItems" :key="item.id">
                ...
                <td>
                  <div class="d-flex">
                    <v-btn color="green" variant="text" @click="editZone(item)" icon>
          ...
          <skeleton-table v-if="isLoading" :rows="5" :columns="2" />
          <empty-state
...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      return this.items.filter((item) => item.zone.toLowerCase().includes(searchTextLower));
    },
  },
  methods: {
    editZone(zone) {
