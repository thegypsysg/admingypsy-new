...
<template>
  <v-container>
          ...
          <v-table class="country-table">
            ...
            <tbody>
              <tr class="country-table-body" v-for="item in filteredItems" :key="item.id">
                ...
                <td>
                  <div class="d-flex">
                    <v-btn color="green" variant="text" @click="editPropertyType(item)" icon>
    ...
    <confirm-dialog
      ...
      @confirm="deletePropertyType"
    />
    <v-dialog persistent width="auto" v-model="isOpenImage">
      <v-card width="750">
        <v-card-title class="upload-title px-6 py-4"> Upload Image - Property Types</v-card-title>
        <v-card-text>
          <image-upload
...
export default {
  ...
  methods: {
    ...
    deleteImageFile() {
      this.isSending = true;
      axios
        .delete(`/four-walls-property-types/${this.propertyDataToImage.id}/image`)
        .then((response) => {
