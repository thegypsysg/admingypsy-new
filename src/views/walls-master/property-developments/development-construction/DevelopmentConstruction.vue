...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      return this.developmentConstructionData.filter((item) =>
  ...
  methods: {
    ...
    getDevelopmentConstructionData() {
      axios
        .get(`/4walls-development-constructions/${this.developmentId}`)
        .then((response) => {
    ...
    deleteImageFile() {
      this.isSending = true;
      axios
        .delete(`/4walls-development-constructions/${this.propertyDataToImage.pdc_id}/image`)
        .then((response) => {
          const data = response.data;
          this.notification.success(data.message);
          this.getDevelopmentConstructionData();
