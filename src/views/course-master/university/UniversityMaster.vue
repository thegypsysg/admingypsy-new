...
<template>
  <v-container>
    <div class="d-flex ml-4 mb-4" style="gap: 40px">
      ...
      <router-link style="color: #940f1b" class="text-decoration-none" to="/university_master">
        ...
        <v-row>
          ...
          <v-col cols="12" md="4">
            <div class="w-100 d-flex align-center">
              ...
              >
                <span class="fi ml-2 pr-4 mr-4" :class="['fi-' + input.country.toLowerCase()]" />
              </div>
              <MazSelect
                v-slot="{ option }"
              ...
              >
                ...
                >
                  <span class="fi" :class="['fi-' + option.value.toLowerCase()]" />
                  <span class="pl-2">
                    {{ option.label }}
                  </span>
                </div>
            ...
            <tbody>
              <template v-for="item in filteredItems" :key="item.id">
                <tr class="country-table-body">
                  ...
                  <td>
                    <div class="d-flex">
                      <v-btn color="green" variant="text" @click="editUniversity(item)" icon>
    ...
    editUniversity(university) {
      ...
      this.input = {
        ...
        country: university.country
          ? this.options.filter((i) => i.label == university.country)[0].value
          : null,
    ...
    saveEdit() {
      if (this.valid) {
        ...
        const payload = {
          ...
          flag:
            'https://flagicons.lipis.dev/flags/4x3/' + this.input.country.toLowerCase() + '.svg',
        };
        axios
        ...
        .then((response) => {
          ...
          this.items = data.map((item) => {
            return {
              ...
              country:
                item.country_name && item.country_name.includes(' (') == false
                  ? item.country_name
                  : item.country_name && item.country_name.includes(' (') == true
                  ? item.country_name.split(' (')[0]
                  : '',
